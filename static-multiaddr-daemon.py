...
def receiver(group):
    ...
    s.setsockopt(socket.SOL_SOCKET, 25, str("br0.2" + '\0').encode('utf-8'))
...
def set_lifetime(iface, address, preferred):
    """
...
def match_ips(ip_output):
    ...
    for entry in IP_ADDR_MATCHER.finditer(ip_output):
        addrs.append({
            ...
            'lft': process_lft(entry.group('preferred')),
        })
    return addrs

def load_addresses_for_iface(iface):
    """
...
def process_prefixes_ifaces(prefixes, iface):
    """
    [{'A': 1,
      'L': 1,
      'R': 0,
    ...
    """
    addresses = load_addresses_for_iface(iface)
    for prefix in prefixes:
        ...
        for address in addresses:
            ...
            if address["lft"] > 0 and prefix["preferred"] == 0:
                set_lifetime(iface, address["address"], '0')
            if address["lft"] == 0 and prefix["preferred"] > 0:
                set_lifetime(iface, address["address"], 'forever')

def act_on_ra(packet):
    ...
    if len(prefixes) > 0:
        process_prefixes_ifaces(prefixes, iface)
    print('=== captured packet END ===')

print('start')
receiver(IPV6_ALLNODES_MCAST)
print('end')
